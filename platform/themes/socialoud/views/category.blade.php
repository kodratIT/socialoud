...
<main class="socialoud-container socialoud-category-page">
    ...
    <section class="socialoud-category-heading">
        <span></span>
        <h1>{!! BaseHelper::clean($category->name) !!}</h1>
    </section>
    ...
    <div class="socialoud-subcategory-slider">
        ...
        <nav class="socialoud-subcategory-nav" aria-label="Subkategori {!! BaseHelper::clean($category->name) !!}">
            <button class="is-active" type="button" data-category-filter data-category-id="{{ $category->getKey() }}" data-category-all="1">Semua</button>
            <button type="button" data-category-filter data-category-id="{{ $category->getKey() }}">{!! BaseHelper::clean($category->name) !!}</button>
            @foreach ($subcategories as $subcategory)
                <button type="button" data-category-filter data-category-id="{{ $subcategory->getKey() }}">{!! BaseHelper::clean($subcategory->name) !!}</button>
            @endforeach
        </nav>
    ...
    <section class="socialoud-category-layout">
        <div class="socialoud-category-main">
            <div class="socialoud-category-list" data-category-post-list data-filter-endpoint="{{ $filterEndpoint }}">
                {!! Theme::partial('category-post-list', ['posts' => $initialPosts, 'category' => $category]) !!}
            </div>

            @if ($categoryPosts->count() > 10 || $hasMorePosts)
                <button
                    class="socialoud-load-more"
                    type="button"
                    data-socialoud-load-more
                    data-next-url="{{ $filterEndpoint }}?all=1&page=2"
                >

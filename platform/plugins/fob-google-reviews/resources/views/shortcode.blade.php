...
@php
    $displayType = setting('google_reviews_display_type', 'carousel');
    $reviewsPerRow = setting('google_reviews_reviews_per_row', '3');
    $customTitle = $title ?? '';
@endphp
<div class="fob-google-reviews-widget fob-gr-display-{{ $displayType }}"
     style="--reviews-per-row: {{ $reviewsPerRow }};"
    ...
    <div class="fob-gr-header">
        ...
        <div class="fob-gr-header-content">
            <h3 class="fob-gr-business-name">{{ $customTitle ?: ($data['place_name'] ?? setting('google_reviews_business_name', 'Our Business')) }}</h3>
    ...
    <div class="fob-gr-reviews-container">
        ...
        @elseif($displayType === 'grid')
            <div class="fob-gr-grid-wrapper">
        @else
            <div class="fob-gr-list-wrapper">
        @endif
                ...
                <div class="fob-gr-review-card">
                    ...
                    @if(!empty($review['text']))
                        @php
                            ...
                            $shouldTruncate = strlen($text) > $textLength;
                            $displayText = $shouldTruncate ? substr($text, 0, $textLength) . '...' : $text;
                        @endphp
                        ...
                        @if($shouldTruncate && setting('google_reviews_show_read_more', true))
    ...
    @if(setting('google_reviews_show_view_all_button', true) && setting('google_reviews_place_id'))
